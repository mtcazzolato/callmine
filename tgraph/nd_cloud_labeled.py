...
# data file must have column "node_ID"
# expected column headers in the file with labels:
NODE_ID = "node_ID"
LABEL = "label"

class nd_cloud_labeled:
    '''
    Expects two data frames: (i) one with all of columns: numerical
    features; and (ii) one with two columns: NODE_ID and LABEL

    and it provides several plots, colored by the provided labels:
    histograms, 2d-contour plots, 3d interactive ones etc
    '''

    def __init__(self, filename, filename_label):
        self.filename = filename
        self.filename_label = filename_label
        self.df = pd.read_csv(filename)
        self.df_label = pd.read_csv(filename_label)
        self.sort_labels()
        self.headers = list (self.df.columns.values)
        self.headers_label = list (self.df_label.columns.values)
        self.num_columns = len(self.headers)
        # get the number of unique labels
        self.labels = self.df_label[LABEL].unique()
        self.num_labels = len(self.labels)

        self.numerical_columns_list = list(range(0, self.num_columns))
    ...
    def sort_labels(self):
        ...
        given by the nodes (i.e., df[NODE_ID])
        """
        self.df_label = self.df.set_index(NODE_ID).join(self.df_label.set_index(NODE_ID))[[LABEL]]
        self.df_label = self.df_label.reset_index()
        return
    
    def get_label_indexes(self, label_value):
        """
        returns the indexes of rows containing the informed label_value
        """
        idx = np.where(self.df_label[LABEL] == label_value)[0].tolist()
    ...
    def histogram_cn(self, column_name: str, print_flag: bool) -> None:
        ...
        fig, ax = plt.subplots(nrows=1, ncols=self.num_labels, sharex=True, sharey=True)

        for i, l in enumerate(self.labels):
            idx_label = self.get_label_indexes(label_value=l)
            n, bins, patches = ax[i].hist(x = pd.Series(data=xp).loc[idx_label],
                                        log=True,
                                        bins='auto')
    ...
    def hexbin_2d(self,
        ...
        cname1 and cname2 with each label value
        """
        ...
        for i, l in enumerate(self.labels):
            idx_label = self.get_label_indexes(l)

            # ready to plot
            img = ax[i].hexbin(pd.Series(data=xp).loc[idx_label],
                               pd.Series(data=yp).loc[idx_label],
                               xscale = xscale,
                               yscale = yscale,
                               # marginals = True,
                               # vmin= 1,
            ...
            ax[i].set_xlabel(cname1 + " (+"  + str(x_offset) + ")")
            ax[i].set_ylabel(cname2 + " (+"  + str(y_offset) + ")")
            cb = plt.colorbar(img, ax=ax[i])
            cb.set_label("log10(N)")
            ax[i].set_title("{} - Label={}".format(self.filename, l))
            ax[i].grid()
...
if __name__ == "__main__":
    ...
    parser.add_argument("filename_label",
                        help="input file with labels csv.gz")
    # parser.add_argument("filename",
    ...
    ndc = nd_cloud_labeled(filename, filename_label)
